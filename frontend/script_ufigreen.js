// Ścieżka do pliku CSV z zapachami UFI GREEN na serwerze
const CSV_URL = 'https://clpgenerator.brzezinski.studio/clp-generator/nowa-wersja/zapachy-ufigreen.csv';
...
fetch(CSV_URL)
    ...
    .then(csvText => {
        ...
        parsed.data.slice(1).forEach(row => {
            ...
            fragranceData[zapach][stezenie] = tekst;
        });
...
function getWarningText() {
    ...
    return (options && options[stezenie]) || ' ';
}
...
function updatePreview() {
  ...
  Nie przesuwaj ani nie przenoś zapalonej świecy. Nie używaj płynów do gaszenia płomienia.`;

    const fixedBaseLimits = [55, 65, 70, 80, 80, 85, 85];
    ...
    const labelBaseLimits = [75, 80, 80, 80, 80, 75, 70];
